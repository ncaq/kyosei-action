...
Version tags such as `@v0.4.0` are safe to use as-is.
...
Do not use `git rev-parse v0.4.0` without `^{commit}`.
...
jobs:
  workflow:
    ...
    uses: ncaq/kyosei-action/.github/workflows/review.yml@v0.4.0
    secrets:
    ...
    steps:
      - uses: actions/checkout@de0fac2e4500dabe0009e67214ff5f5447ce83dd # v6.0.2
        with:
          ...
          fetch-depth: 50
      - uses: ncaq/kyosei-action@v0.4.0
        with:
          claude_code_oauth_token: ${{ secrets.CLAUDE_CODE_OAUTH_TOKEN }}
```
